...
Game game;
HtmlElement playerOne, playerTwo, nextInLine;
...
void prepareGame() {
  game = new Game.withCards(2, 8);
  playerOne = querySelector('#playerOne');
  playerTwo = querySelector('#playerTwo');
  nextInLine = querySelector('#nextInLine');
  var container = querySelector('#board');
...
void updateScore() {
  var nameOfPlayers = ["Pepa", "Jirka"];
  playerOne.text = "${nameOfPlayers[0]}: ${game.score[0]}";
  playerTwo.text = "${nameOfPlayers[1]}: ${game.score[1]}";
  nextInLine.text = nameOfPlayers[game.playerOnTurn];
}
